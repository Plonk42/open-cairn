 ...
 *
 * Bins points into a regular XY grid (cell size ≈ point spacing), keeps the
 * *minimum* Z per cell (true ground when applied to ground+vegetation
 * mixed input — vegetation hits are pushed above ground returns), fills
 * small holes by 3×3 mean of valid neighbours, and emits two indexed
 ...
 * free of the dark-stripe artefact that plagues 2.5D Delaunay on cliffs.
 *
 * Trade-off vs Delaunay: regular cell density wastes vertices on flat
 * areas; cliffs become 1-cell-wide near-vertical quads (still 2.5D, so a
 * cave UNDER a cliff edge is not represented — use the `voxel` method for
 * that). Quads spanning a no-data cell (after hole-fill) are dropped, so
 * large gaps stay open instead of being bridged.
 */
 ...
 *                    extends valid coverage by 1 cell. Large gaps stay
 *                    empty (drop those quads).
 */
export function buildGridMesh(
    ...
    for (let v = 0; v < vCount; v++) {
        const nz = Math.max(-1, Math.min(1, outNormals[v * 3 + 2]));
    ...
    //    longest edge (incl. diagonal) is below a Z-cutoff. The Z cutoff
    //    keeps cliffs (single near-vertical quad) while dropping bridges
    //    across deep canyons / no-data trenches.
    const maxQuadDz = Math.max(50, cellSize * 200); // very lax — cliffs OK
    const idx: number[] = [];
