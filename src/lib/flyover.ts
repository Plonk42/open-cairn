...
// The centre is smoothed far less than the heading: a wide window cuts across
// switchbacks by kilometres, which drops the progress marker off the screen.
const HEADING_SMOOTH_SECONDS = 5;
const CENTER_SMOOTH_SECONDS = 1.2;
const PATH_SMOOTH_PASSES = 2;
const MAX_AHEAD_SHARE = 0.4;              // anticipation cap for very short routes
...
/**
 * Every series below is indexed by distance along the REAL route, so the camera
 * and the progress marker always describe the same point of the itinerary — a
 * smoothed path has its own, much shorter, arc length.
 */
function indexAt(count: number, step: number, distance: number): { lo: number; hi: number; t: number } {
    const x = Math.max(0, Math.min(distance / step, count - 1));
    const lo = Math.floor(x);
    return { lo, hi: Math.min(lo + 1, count - 1), t: x - lo };
}

function pointOn(path: LngLatTuple[], step: number, distance: number): LngLatTuple {
    const { lo, hi, t } = indexAt(path.length, step, distance);
    const a = path[lo];
    const b = path[hi];
    return [a[0] + (b[0] - a[0]) * t, a[1] + (b[1] - a[1]) * t];
}

function valueOn(values: number[], step: number, distance: number): number {
    const { lo, hi, t } = indexAt(values.length, step, distance);
 ...
 */
function smoothPath(samples: LngLatTuple[], radius: number): LngLatTuple[] {
    ...
    return lng.map((x, i): LngLatTuple => [x, lat[i]]);
}
...
function buildAltitudeProfile(
    ...
    const raw: number[] = [];
    if (profile.length < 2) {
        for (let i = 0; i < count; i++) raw.push(fallback + CAMERA_CLEARANCE_METERS);
        return raw;
    }
    let j = 0;
    for (let i = 0; i < count; i++) {
        ...
        raw.push(a.elevation + (b.elevation - a.elevation) * t + CAMERA_CLEARANCE_METERS);
    }
...
/**
 * Ground speed of the camera itself, which is what sets the on-screen pace. The
 * smoothed path cuts across switchbacks, so on a winding route it is much shorter
 * than the itinerary and the camera crawls where the runner sprints.
 */
function pathSpeed(path: LngLatTuple[], totalDurationMs: number): number {
    let length = 0;
    for (let i = 1; i < path.length; i++) length += distanceMeters(path[i - 1], path[i]);
    return length / ((totalDurationMs / 1000) * (1 - EASE_EDGE));
}
...
export class FlyoverController {
    ...
    start(map: MaplibreMap, coordinates: LngLatTuple[], options: FlyoverOptions): void {
        ...
        const maxAhead = route.total * MAX_AHEAD_SHARE;
        const lookAhead = Math.min(cruiseSpeed * LOOK_AHEAD_SECONDS, maxAhead);
        const headingAhead = Math.min(cruiseSpeed * BEARING_LOOK_AHEAD_SECONDS, maxAhead);

        const samples = resampleByDistance(route, PATH_STEP_METERS);
        const step = route.total / (samples.length - 1);
        const centerPath = smoothPath(samples, Math.round((cruiseSpeed * CENTER_SMOOTH_SECONDS) / step));
        const headingRadius = Math.round((cruiseSpeed * HEADING_SMOOTH_SECONDS) / step);
        const headingPath = smoothPath(samples, headingRadius);

        const zoom = zoomForSpeed(map, pathSpeed(centerPath, totalDurationMs), cleaned[0][1]);
        // With 3D terrain off the center plane is sea level, as MapLibre expects.
        const altitudes = map.getTerrain()
            ? buildAltitudeProfile(samples.length, step, profile, headingRadius, map.queryTerrainElevation(samples[0]) ?? 0)
            : samples.map(() => 0);

        // Anticipation shrinks linearly along the route so the camera lands on the
        // finish at the same instant as the marker. Clamping it there instead would
        // freeze the camera for the last seconds while the marker still creeps on.
        const centerShrink = 1 - lookAhead / route.total;
        const headingShrink = 1 - headingAhead / route.total;
        ...
        const frame = (distance: number) => {
            const at = distance * centerShrink + lookAhead;
            const from = distance * headingShrink;
            map.jumpTo({
                center: pointOn(centerPath, step, at),
                bearing: bearing(pointOn(headingPath, step, from), pointOn(headingPath, step, from + headingAhead)),
                pitch,
                zoom,
                ...
                elevation: valueOn(altitudes, step, at),
            });
        };
        ...
        const animate = (timestamp: number) => {
            ...
            const distance = easedDistanceFraction(progress, EASE_EDGE) * route.total;
            frame(distance);
            onProgress?.(distance);
