...
export class MultiplyBlendLayer implements maplibregl.CustomLayerInterface {
    ...
    private static readonly SUBDIV = 8;
    ...
    private terrainListener: (() => void) | null = null;
    ...
    onAdd(map: maplibregl.Map, gl: WebGLRenderingContext | WebGL2RenderingContext): void {
        ...
        // Whenever the terrain DEM source loads a new tile, our cached meshes
        // need to be re-sampled against the now-known elevation.
        this.terrainListener = () => {
            for (const t of this.tiles.values()) t.meshDirty = true;
            map.triggerRepaint();
        };
        map.on('data', (e: maplibregl.MapDataEvent) => {
            const ev = e as unknown as { sourceId?: string; dataType?: string };
            if (ev.dataType === 'source' && ev.sourceId === 'terrain') {
                this.terrainListener?.();
            }
        });
    }
