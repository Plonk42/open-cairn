...
/**
 * "Point de vue" toggle: a three-state button, off → armed → standing. Armed,
 * the next click on the map plants the eye on the ground there; from then on the
 * camera only turns, as if you were standing on that spot. Clicking again gives
 * the ordinary camera back.
 *
...
function ViewpointTopBarButton({ needsTerrain }: Readonly<{ needsTerrain: boolean }>) {
    ...
    return (
        <button
            ...
            className={`inline-flex items-center gap-1.5 rounded-md px-3 py-1.5 text-xs font-medium ring-1 transition disabled:cursor-not-allowed disabled:opacity-40 ${active || picking
                ? 'bg-green-600/10 text-green-700 ring-green-600/30 dark:bg-emerald-500/20 dark:text-emerald-200 dark:ring-emerald-400/40'
                : 'bg-black/5 text-slate-600 ring-black/5 hover:bg-black/10 dark:bg-white/5 dark:text-slate-200 dark:ring-white/15 dark:hover:bg-white/10'}`}
        >
