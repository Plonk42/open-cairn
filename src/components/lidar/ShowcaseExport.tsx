...
function buildScene(id: string, title: string, description: string): ShowcaseScene | null {
    ...
    return {
        ...
        captures: visibleClouds.some((c) => c.capture) ? visibleClouds.map((c) => c.capture ?? null) : undefined,
    };
}
        ...
        try {
            ...
            if (target.local) {
                await saveScene(
                    ...
                    { camera: scene.camera, ambiance: scene.ambiance, shaded: scene.shaded, mesh: scene.mesh, extraClouds: scene.extraClouds, captures: scene.captures },
                    thumb,
                );
            }
